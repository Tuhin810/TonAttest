...
describe("failing closed", () => {
  it("returns 503 when the provider is down, never a false ineligible", async () => {
    ...
    h.setProvider({
      getAccountEvents: async () => {
        throw new TonAttestError("PROVIDER_UNAVAILABLE", "tonapi is down");
      },
    });
  ...
  it("returns 503 when the provider rate-limits us", async () => {
    ...
    h.setProvider({
      getAccountEvents: async () => {
        throw new TonAttestError("PROVIDER_RATE_LIMITED", "429");
      },
    });
  ...
  it("returns 502 for a malformed provider response, marked non-retryable", async () => {
    ...
    h.setProvider({
      getAccountEvents: async () => {
        throw new TonAttestError("PROVIDER_MALFORMED_RESPONSE", "bad shape", {
          retryable: false,
        });
      },
    });
  ...
  it("issues no attestation on any of those failures", async () => {
    ...
    h.setProvider({
      getAccountEvents: async () => {
        throw new TonAttestError("PROVIDER_UNAVAILABLE", "down");
      },
    });
