...
- `tonattest_verify_duration_seconds{quantile="0.95"}` — target under 2s warm
- `tonattest_provider_errors_total` — upstream trouble; expect matching 503s
- `tonattest_unknown_actions_total` — **the early warning that STON.fi shipped a
  contract change we do not decode yet.** A rising rate here means eligibility
  answers are quietly degrading. Alert on it.
- `tonattest_rate_limited_total` — an integrator hitting their budget
...
pnpm --filter @tonattest/service migrate    # idempotent
```
...
1. `tonattest_unknown_actions_total` rises. That is the signal.
