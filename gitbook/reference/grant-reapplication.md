...
Signed with the same Ed25519 mechanism. Verified offline with the same public-key model already documented in the [attestation spec](../application-1/attestation-spec.md). The signer, the canonicalization, the SDK's `verifyAttestation()` — all reusable without modification. What's new is the *subject* of the attestation (an asset, not a wallet-campaign pair), a public read endpoint, and a real revocation lifecycle.
