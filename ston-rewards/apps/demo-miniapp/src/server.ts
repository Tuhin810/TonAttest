...
import { StonRewards, verifyAttestation, type Attestation } from "@ston-rewards/sdk";
import { PointsLedger } from "./points.js";

const API_KEY = process.env["STON_REWARDS_API_KEY"];
const CAMPAIGN_ID = process.env["STON_REWARDS_CAMPAIGN_ID"];
...
const PUBLIC_KEY = process.env["STON_REWARDS_PUBLIC_KEY"];
const POINTS_PER_CLAIM = 100;
const PORT = Number(process.env["PORT"] ?? 3000);

if (!API_KEY || !CAMPAIGN_ID || !PUBLIC_KEY) {
  console.error(
    "Set STON_REWARDS_API_KEY, STON_REWARDS_CAMPAIGN_ID and STON_REWARDS_PUBLIC_KEY.\n" +
      "Get all three from: pnpm --filter @ston-rewards/service provision \"Demo\"",
  );
  process.exit(2);
}
