...
import { generateKeypair, signAttestation, toHex } from "@ston-rewards/attest";
import {
  ApiResponseError,
  InvalidInputError,
  NetworkError,
...
describe("construction", () => {
  it("requires an API key", () => {
    expect(() => new StonRewards({ apiKey: "" })).toThrow(InvalidInputError);
  });

  it("explains what to do when no fetch exists", () => {
    expect(
      () => new StonRewards({ apiKey: "k", fetchImpl: undefined as never }),
    ).not.toThrow();
  });

  it("trims a trailing slash off the base URL", async () => {
    const fetchImpl = vi.fn(async () => json({ campaigns: [] }));
    const sdk = new StonRewards({
      apiKey: "k",
      baseUrl: "https://api.example.com/",
      fetchImpl,
    });
